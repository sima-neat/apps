...
int main(int argc, char** argv) {
  ...
  const std::string kExpectedModel = "yolov5s_face_raw_split_mpk.tar.gz";
  ...
  std::vector<std::string> label_search = {
    "examples/face-detection/yolov5-face/common/face_label.txt",
    example_dir + "/common/face_label.txt",
  };
  for (const auto& p : label_search) {
    if (!labels_file.empty()) break;
    if (fs::exists(p)) {
      labels_file = p;
    }
  }
  if (labels_file.empty()) {
    return skip_or_fail(
        "common/face_label.txt not found; set SIMANEAT_APPS_TEST_LABELS_FILE "
  ...
  auto out_dir = create_temp_dir("yolov5-face_e2e_");
  ...
  if (r.exit_code != 0) {
    ...
    std::cout << "[OK] yolov5-face produced " << count_files(out_dir)
              << " output files\n";
  }
