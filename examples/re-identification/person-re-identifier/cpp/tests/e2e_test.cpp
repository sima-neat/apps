// E2E test for person-re-identifier.
...
int main(int argc, char** argv) {
  ...
  // Case 1: cosine metric, default output type (both).
  {
    const std::string out_dir =
        create_test_output_dir("person-re-identifier", "test_full_pipeline_cosine");
    if (out_dir.empty()) {
      return 1;
    }
  ...
  {
    const std::string out_dir =
        create_test_output_dir("person-re-identifier", "test_output_type_json_only");
    if (out_dir.empty()) {
      return 1;
    }
    const fs::path config_path = fs::path(out_dir).parent_path() / "config.yaml";
