...
| Languages | C++, Python |
| Status | experimental |
| Binary Name | human-pose-estimator |
| Model | open_pose |
...
## Preview
![Human pose estimator preview](../../../assets/portal/pose-estimation/human-pose-estimator/image.png)
...
### C++
- Invocation:
  `./build/examples/pose-estimation/human-pose-estimator/human-pose-estimator [--config <path>] [--profile]`
- Optional arguments:
  - `--config <path>` - Path to YAML configuration. Default: `examples/pose-estimation/human-pose-estimator/common/config.yaml`
  - `--profile` - Enable performance profiling (reports end-to-end and model-inference timing)

### Python
- Invocation:
  `python examples/pose-estimation/human-pose-estimator/python/main.py [--config <path>] [--profile]`
- Optional arguments:
  - `--config <path>` - Path to YAML configuration. Default: `examples/pose-estimation/human-pose-estimator/common/config.yaml`
  - `--profile` - Enable performance profiling (reports end-to-end and model-inference timing)
...
Binary output:
```bash
./build/examples/pose-estimation/human-pose-estimator/human-pose-estimator
```

### Build This Example Directly With CMake
```bash
cd <apps-repo-root>/examples/pose-estimation/human-pose-estimator
cmake -S cpp -B build
cmake --build build -j
```
...
```bash
./build/examples/pose-estimation/human-pose-estimator/human-pose-estimator \
  --config examples/pose-estimation/human-pose-estimator/common/config.yaml
```

### Python
```bash
source ~/pyneat/bin/activate
pip install -r examples/pose-estimation/human-pose-estimator/python/requirements.txt
python examples/pose-estimation/human-pose-estimator/python/main.py \
  --config examples/pose-estimation/human-pose-estimator/common/config.yaml
```
