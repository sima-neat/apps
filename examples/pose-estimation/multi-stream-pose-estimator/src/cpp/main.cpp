...
void draw_poses(cv::Mat& frame, const std::vector<Pose>& poses, double min_visibility) {
  ...
  for (const auto& pose : poses) {
    const cv::Point top_left(std::max(0, static_cast<int>(pose.x1)),
                             std::max(0, static_cast<int>(pose.y1)));
    const cv::Point bottom_right(std::min(frame.cols - 1, static_cast<int>(pose.x2)),
                                 std::min(frame.rows - 1, static_cast<int>(pose.y2)));
    ...
    for (const auto& [start, end] : kCocoSkeleton) {
      ...
      if (visible(a) && visible(b)) {
        cv::line(frame, cv::Point(static_cast<int>(a.x), static_cast<int>(a.y)),
                 cv::Point(static_cast<int>(b.x), static_cast<int>(b.y)), color, 2);
      }
    }
    for (const auto& point : pose.keypoints) {
      if (visible(point)) {
        cv::circle(frame, cv::Point(static_cast<int>(point.x), static_cast<int>(point.y)), 3, color,
                   -1);
      }
    }
  }
...
void connect_stream_graph(AppRuntime& app, const AppConfig& cfg, const StreamRuntime& stream,
  ...
  auto decoded_branch =
      save_debug_frames
          ? simaai::neat::graphs::Branch("decoded", {"estimator_frame", "debug_frame"})
...
void send_metadata(StreamRuntime& stream, const AppConfig& cfg, const simaai::neat::Sample& sample,
  ...
  std::string err;
  if (!stream.metadata_sender->send_metadata("pose-estimation", data_json, timestamp_ms, frame_id,
                                             &err)) {
...
void maybe_save_debug_frame(const AppConfig& cfg, const StreamRuntime& stream, const cv::Mat* frame,
  ...
  draw_poses(bgr, poses, cfg.min_keypoint_visibility);
...
void process_output_sample(StreamRuntime& stream, const AppConfig& cfg,
  ...
  const auto poses = decode_poses(sample, stream.frame_w, stream.frame_h, cfg.max_poses);
  ...
  if (!warming_up) {
    ...
    if (save_frames_enabled(cfg)) {
      maybe_save_debug_frame(
          cfg, stream, stream.latest_debug_frame ? &*stream.latest_debug_frame : nullptr, poses);
    }
