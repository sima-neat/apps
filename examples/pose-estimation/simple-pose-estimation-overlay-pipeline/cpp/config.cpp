...
#else
  return fs::path(
      "examples/pose-estimation/simple-pose-estimation-overlay-pipeline/common/config.yaml");
#endif
}
...
AppConfig load_app_config(const std::filesystem::path& path) {
  ...
  cfg.decode.paf_score =
      optional_double(raw, "decode.paf_score", 0.05, "decode.paf_score");
  cfg.decode.paf_success_ratio = optional_double(
      raw, "decode.paf_success_ratio", 0.8, "decode.paf_success_ratio");
  cfg.decode.paf_samples =
      optional_int(raw, "decode.paf_samples", 10, "decode.paf_samples");
  cfg.decode.min_valid_joints = optional_int(
      raw, "decode.min_valid_joints", 3, "decode.min_valid_joints");
  cfg.decode.min_avg_person_score = optional_double(
      raw, "decode.min_avg_person_score", 0.2, "decode.min_avg_person_score");
