...
int main(int argc, char** argv) {
  ...
  if (!rtsp_url) {
    return skip_or_fail("SIMANEAT_APPS_TEST_RTSP_URL is required for instance-segmenter e2e");
  }
  ...
  const std::string output_dir = create_test_output_dir("instance-segmenter", "test_full_pipeline");
  if (output_dir.empty()) {
    return 1;
  }

  const fs::path config_path = fs::path(output_dir).parent_path() / "config.yaml";
  const sima_examples::ScalarConfig& common = example_common_config("instance-segmenter");
  ...
  const int save_every = e2e_int("instance-segmenter", "testing.e2e.output", "save_every");
  const int total_saved_frames =
      e2e_int("instance-segmenter", "testing.e2e.output", "total_saved_frames");

  {
    ...
    config_file << "source:\n"
                ...
                << common.string_or(
                       "model.labels",
                       "examples/segmentation/instance-segmenter/src/common/coco_label.txt")
                << "\n"
                << "inference:\n"
                ...
                << "  draw_boxes: "
                << (common.bool_or("output.draw_boxes", true) ? "true" : "false") << "\n"
                << "  insight:\n"
                << "    host: " << insight_host << "\n"
  ...
  if (result.exit_code != 0) {
    ...
    if (files < total_saved_frames) {
      ...
      std::cout << "[OK] instance segmenter produced " << files << " sampled output files\n";
    }
  }
