...
class AppConfig:
    ...
    frames: int
    insight_host: str
    insight_video_port_base: int
    insight_metadata_port_base: int
    fps: int
    bitrate_kbps: int
    save_every: int
    profile: bool
...
def load_app_config(path: str | Path) -> AppConfig:
    ...
    if cfg.insight_video_port_base <= 0:
        raise ValueError("insight.video_port_base must be > 0")
    if cfg.insight_metadata_port_base <= 0:
        raise ValueError("insight.metadata_port_base must be > 0")
    if cfg.frames < 0:
