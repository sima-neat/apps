...
from .sample_utils import (
    ...
    make_insight_tracking_detection,
    parse_bbox_payload,
...
def run_app(cfg: AppConfig) -> int:
    ...
    for stream in streams:
        print(
            ...
            f"{stream.url} -> insight://{cfg.insight_host} "
            f"video={insight_video_port_for_stream(cfg.insight_video_port_base, stream.index)} "
            f"metadata={insight_metadata_port_for_stream(cfg.insight_metadata_port_base, stream.index)}"
        )
