# Performant Multi-Camera People Detection, Tracking, and Insight Rendering
...
| Languages | C++, Python |
| Status | experimental |
| Binary Name | multi-camera-people-detection-and-tracking-insight |
| Model | yolo_v8m |

## Concept
Multi-camera people detection and tracking example with RTSP inputs, mixed-resolution support, per-stream worker threads, Insight live video plus metadata output, and optional sampled overlay saves.
...
![Multi-camera Insight tracking demo](../../../assets/portal/tracking/multi-camera-people-detection-and-tracking-insight/image.png)
...
- Invocation:
  ```bash
  ./build/examples/tracking/multi-camera-people-detection-and-tracking-insight/multi-camera-people-detection-and-tracking-insight \
    --config examples/tracking/multi-camera-people-detection-and-tracking-insight/common/config.yaml
  ```
- Required arguments:
  None.
- Optional arguments:
...
- Invocation:
  ```bash
  python3 examples/tracking/multi-camera-people-detection-and-tracking-insight/python/main.py \
    --config examples/tracking/multi-camera-people-detection-and-tracking-insight/common/config.yaml
  ```
- Required arguments:
  None.
...
```bash
cd <apps-repo-root>
cmake -S examples/tracking/multi-camera-people-detection-and-tracking-insight/cpp -B build/multi-camera-people-detection-and-tracking-insight
cmake --build build/multi-camera-people-detection-and-tracking-insight -j
```

Binary output:
```bash
./build/multi-camera-people-detection-and-tracking-insight/multi-camera-people-detection-and-tracking-insight
```

## Run
Before running either entrypoint, edit `examples/tracking/multi-camera-people-detection-and-tracking-insight/common/config.yaml` and replace the placeholder values in:
...
pip install -r examples/tracking/multi-camera-people-detection-and-tracking-insight/python/requirements.txt
```
