#include "examples/tracking/multi-stream-people-tracker/cpp/utils/workers_api.cpp"

#include "examples/tracking/multi-stream-people-tracker/cpp/utils/image_utils_api.cpp"
#include "examples/tracking/multi-stream-people-tracker/cpp/utils/pipeline_api.cpp"
...
int run_app(const AppConfig& cfg) {
  ...
  for (const auto& stream : streams) {
    std::cout << "[stream " << stream.index << "] " << stream.probe.width << "x"
              ...
              << insight_video_port_for_stream(cfg.insight_video_port_base, stream.index)
              << " metadata="
              << insight_metadata_port_for_stream(cfg.insight_metadata_port_base, stream.index)
              << "\n";
  }
