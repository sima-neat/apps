#include "examples/tracking/multi-stream-people-tracker/cpp/utils/config_api.cpp"
...
#else
  return fs::path("examples/tracking/multi-stream-people-tracker/common/config.yaml");
#endif
}
...
AppConfig load_app_config(const std::filesystem::path& path) {
  ...
  cfg.insight_video_port_base =
      optional_int(raw, "output.insight.video_port_base", 9000, "insight.video_port_base");
  cfg.insight_metadata_port_base =
      optional_int(raw, "output.insight.metadata_port_base", 9100, "insight.metadata_port_base");
  ...
  if (cfg.insight_video_port_base <= 0) {
    throw std::runtime_error("insight.video_port_base must be > 0");
  }
  if (cfg.insight_metadata_port_base <= 0) {
    throw std::runtime_error("insight.metadata_port_base must be > 0");
  }
  if (cfg.frames < 0) {
