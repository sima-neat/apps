...
struct AppConfig {
  ...
  int max_inflight_total = 4;
  int num_classes = 1;
  int target_class_id = 0;
  std::string target_label = "drone";
  double min_score = 0.05;
  double nms_iou = 0.60;
  int max_detections = 100;
  bool profile = false;
  int warmup_frames = 30;
  float tracker_high_score = 0.20f;
  float tracker_new_track_score = 0.30f;
  float tracker_iou_threshold = 0.05f;
  float tracker_max_center_distance = 3.0f;
  float tracker_velocity_momentum = 0.75f;
  int tracker_max_missing = 30;
  int tracker_min_confirmed_hits = 2;
  int tracker_max_active = 256;
...
void validate_config(const AppConfig& cfg) {
  ...
  sima_examples::require(cfg.tracker_max_active >= 1, "tracking.max_active_tracks must be >= 1");
  const int max_port_base = 65535 - static_cast<int>(cfg.rtsp_urls.size() - 1);
  const std::string port_range = " must be between 1 and " + std::to_string(max_port_base) +
                                 " for " + std::to_string(cfg.rtsp_urls.size()) + " streams";
  sima_examples::require(cfg.video_port_base > 0 && cfg.video_port_base <= max_port_base,
                         "output.insight.video_port_base" + port_range);
  sima_examples::require(cfg.metadata_port_base > 0 && cfg.metadata_port_base <= max_port_base,
                         "output.insight.metadata_port_base" + port_range);
  if (cfg.video_enabled) {
    const int last_port_offset = static_cast<int>(cfg.rtsp_urls.size() - 1);
    const int video_port_end = cfg.video_port_base + last_port_offset;
    const int metadata_port_end = cfg.metadata_port_base + last_port_offset;
    sima_examples::require(
        video_port_end < cfg.metadata_port_base || metadata_port_end < cfg.video_port_base,
        "output.insight video and metadata port ranges must not overlap when "
        "output.video_enabled is true");
  }
...
AppConfig load_app_config(const fs::path& config_path) {
  ...
  cfg.max_inflight_total = raw.int_or("inference.max_inflight_total", 4);
  cfg.num_classes = raw.int_or("inference.num_classes", 1);
  cfg.target_class_id = raw.int_or("inference.target_class_id", 0);
  cfg.target_label = raw.string_or("inference.target_label", "drone");
  cfg.min_score = raw.double_or("inference.min_score", 0.05);
  ...
  cfg.tracker_high_score =
      static_cast<float>(raw.double_or("tracking.high_score_threshold", cfg.min_score));
  cfg.tracker_new_track_score =
      static_cast<float>(raw.double_or("tracking.new_track_threshold", cfg.tracker_high_score));
  cfg.tracker_iou_threshold = static_cast<float>(
      raw.double_or("tracking.match_iou_threshold", 0.05));
  cfg.tracker_max_center_distance =
      static_cast<float>(raw.double_or("tracking.max_center_distance", 3.0));
  cfg.tracker_velocity_momentum =
      static_cast<float>(raw.double_or("tracking.velocity_momentum", 0.75));
  cfg.tracker_max_missing = raw.int_or("tracking.max_missing_frames", 30);
  cfg.tracker_min_confirmed_hits = raw.int_or("tracking.min_confirmed_hits", 2);
  cfg.tracker_max_active = raw.int_or("tracking.max_active_tracks", 256);
  cfg.tracker_center_distance_enabled = raw.bool_or("tracking.center_distance_enabled", true);
...
build_source_options(const AppConfig& cfg, const std::string& url, int& fps_out, int& width_out,
  ...
  if (width_out > 0 && height_out > 0 && fps_out > 0) {
    ...
    fps_out = configure_output_fps(opt, fps_out, cfg.fps);
  }
  return opt;
}
...
build_decode_graph(const std::string& input_name,
  ...
  append_decode_output_nodes(decode, opt);
  return decode;
}
...
StreamRuntime build_stream_runtime(const AppConfig& cfg, int stream_index, const std::string& url) {
  ...
  tracker_config.high_score_threshold = cfg.tracker_high_score;
  tracker_config.new_track_threshold = cfg.tracker_new_track_score;
  tracker_config.match_iou_threshold = cfg.tracker_iou_threshold;
  tracker_config.max_center_distance = cfg.tracker_max_center_distance;
  tracker_config.velocity_momentum = cfg.tracker_velocity_momentum;
  tracker_config.max_missing_frames = cfg.tracker_max_missing;
  tracker_config.min_confirmed_hits = cfg.tracker_min_confirmed_hits;
  tracker_config.max_active_tracks = cfg.tracker_max_active;
  tracker_config.center_distance_enabled = cfg.tracker_center_distance_enabled;
  runtime.tracker = ObjectTracker(tracker_config);
...
std::optional<cv::Mat> take_debug_frame(StreamRuntime& stream,
                                        const simaai::neat::Sample& detection_sample) {
  const auto match = std::find_if(stream.debug_frames.begin(), stream.debug_frames.end(),
                                  [&detection_sample](const DebugFrame& frame) {
                                    return sample_identities_correlate(frame.frame_id, frame.pts_ns,
                                                                       detection_sample.frame_id,
                                                                       detection_sample.pts_ns);
                                  });
  if (match == stream.debug_frames.end()) {
    return std::nullopt;
  }
  cv::Mat bgr = std::move(match->bgr);
  stream.debug_frames.erase(match);
  return bgr;
}
...
void process_output_sample(StreamRuntime& stream, const AppConfig& cfg,
  ...
  if (!warming_up) {
    ...
    if (save_frames_enabled(cfg)) {
      auto debug_frame = take_debug_frame(stream, sample);
      maybe_save_debug_frame(cfg, stream, debug_frame ? &*debug_frame : nullptr, tracks);
    }
...
void drain_debug_frames(AppRuntime& app, const AppConfig& cfg) {
  ...
  for (auto& stream : app.streams) {
    ...
    for (;;) {
      ...
      if (sample.frame_id < 0 && sample.pts_ns < 0) {
        continue;
      }
      constexpr std::size_t kMaxPendingDebugFrames = 32;
      if (stream.debug_frames.size() >= kMaxPendingDebugFrames) {
        stream.debug_frames.pop_front();
      }
      stream.debug_frames.push_back(DebugFrame{sample.frame_id, sample.pts_ns, std::move(bgr)});
    }
  }
}
...
bool process_run_once(AppRuntime& app, const AppConfig& cfg, const std::string& output_name) {
  ...
  if (!pull_status_has_sample(status, output_name, pull_error, app.run.last_error())) {
    return false;
  }
