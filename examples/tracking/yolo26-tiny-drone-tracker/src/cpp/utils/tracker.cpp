...
std::vector<TrackedDetection> ObjectTracker::update(const std::vector<Detection>& detections,
  ...
  const auto associate = [&](const std::vector<int>& detection_indices, bool confirmed_only) {
    ...
    for (const auto& [track_id, track] : impl_->tracks) {
      if (matched_tracks.count(track_id) != 0 ||
          (confirmed_only && track.hits < config_.min_confirmed_hits)) {
        continue;
      }
  ...
  associate(high_detections, false);
  // Low-score observations can recover confirmed tracks, but cannot confirm
  // tentative identities or create new ones.
  associate(low_detections, true);
  ...
  for (const int detection_index : high_detections) {
    ...
    if (impl_->tracks.size() >= static_cast<std::size_t>(config_.max_active_tracks)) {
      break;
    }
