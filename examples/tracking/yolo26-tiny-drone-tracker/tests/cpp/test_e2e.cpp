// E2E test for yolo26-tiny-drone-tracker.
...
constexpr const char* kExampleName = "yolo26-tiny-drone-tracker";
...
int run_source_case(const std::string& binary, const std::string& model_path,
  ...
  write_e2e_config(kExampleName, config_path,
                   {{"model.path", model_path},
                    ...
                    {"tracking.high_score_threshold", "0.20"},
                    {"tracking.new_track_threshold", "0.30"},
                    {"tracking.match_iou_threshold", "0.05"},
                    {"tracking.center_distance_enabled", "true"},
                    {"tracking.max_center_distance", "3.0"},
  ...
  if (result.exit_code != 0) {
    ...
    if (files < total_saved_frames) {
      ...
      std::cout << "[OK] " << source_case.codec << " YOLO26 tiny-drone tracker produced " << files
...
int main(int argc, char** argv) {
  ...
  if (cases_run == 0) {
    if (require_e2e_mode()) {
      std::cerr << "[FAIL] no YOLO26 tiny-drone tracker RTSP e2e URLs configured\n";
      return 1;
    }
    std::cerr << "[SKIP] no YOLO26 tiny-drone tracker RTSP e2e URLs configured\n";
    return kSkipCode;
  }
