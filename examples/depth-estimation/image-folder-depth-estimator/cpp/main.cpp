/**
 * @example image-folder-depth-estimator.cpp
 * Minimal Depth Anything V2 pipeline: infer depth for every image in a folder.
 *
 * Usage: image-folder-depth-estimator [--config <path>]
 */
#include "neat.h"
...
static Config load_config(const fs::path& path) {
  ...
  cfg.output_dir = raw.string_or("io.output_dir", "sandbox/image-folder-depth-estimator");
