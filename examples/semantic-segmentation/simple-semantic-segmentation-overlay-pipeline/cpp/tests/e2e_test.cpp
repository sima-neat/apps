...
int main(int argc, char** argv) {
  ...
  if (fs::exists(models_dir)) {
    for (auto& entry : fs::directory_iterator(models_dir)) {
      ...
      if (name.find("hrnet48") != std::string::npos && name.find(".tar.gz") != std::string::npos) {
  ...
  if (model_path.empty()) {
    return skip_or_fail("FCN-HRNet model (.tar.gz) not found under SIMANEAT_APPS_TEST_MODELS_DIR");
  }
