...
int main(int argc, char** argv) {
  ...
  if (!rtsp_url) {
    return skip_or_fail("SIMANEAT_APPS_TEST_RTSP_URL is required for single-stream e2e");
  }

  const char* models_dir_raw = env_or_null("SIMANEAT_APPS_TEST_MODELS_DIR");
  const std::string models_dir = models_dir_raw ? models_dir_raw : "assets/models";
  const std::string model_path = configured_model_path("single-stream-object-detector", models_dir);
  ...
  const std::string output_dir =
      create_test_output_dir("single-stream-object-detector", "test_full_pipeline");
  if (output_dir.empty()) {
    return 1;
  }
  ...
  if (result.exit_code != 0) {
    ...
    if (files < total_saved_frames) {
      ...
      std::cout << "[OK] single-stream object detector produced " << files
