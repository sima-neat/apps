...
SourceGeometry probe_ffprobe_geometry(const AppConfig& cfg) {
  ...
  sima_examples::single_stream_object_detector::FfprobeCommandOptions options;
  options.rtsp_source = cfg.source_type == SourceType::Rtsp;
  options.tcp = cfg.tcp;
  options.tls_verify = cfg.ssl_strict;
  const std::string command =
      sima_examples::single_stream_object_detector::build_ffprobe_geometry_command(cfg.source_url,
                                                                                   options);
