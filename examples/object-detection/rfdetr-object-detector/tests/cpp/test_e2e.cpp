...
int main(int argc, char** argv) {
  ...
  if (rtsp_url == nullptr) {
    return skip_or_fail("SIMANEAT_TEST_RTSP_H264_URL is required for RF-DETR e2e");
  }
  ...
  for (const auto& variant : variants) {
    ...
    const std::string output_dir =
        create_test_output_dir("rfdetr-object-detector", "test_" + variant.name);
    if (output_dir.empty()) {
      return 1;
    }
    ...
    if (process.exit_code != 0) {
      std::cerr << "[FAIL] RF-DETR " << variant.name << " exited with " << process.exit_code
                << "\nstdout:\n"
                << process.stdout_text << "\nstderr:\n"
                << process.stderr_text << "\n";
      return 1;
    ...
    if (!metadata.success || !valid_objects(metadata)) {
      std::cerr << "[FAIL] invalid RF-DETR " << variant.name << " metadata: " << metadata.error
                << "\n";
      return 1;
    }
    remove_dir(output_dir);
  }
  std::cout << "[OK] RF-DETR Small and Medium published valid Insight metadata\n";
  return 0;
}
