...
AppConfig load_app_config(const std::filesystem::path& path) {
  ...
  cfg.optiview_video_port_base =
      optional_int(raw, "output.optiview.video_port_base", 9000, "output.optiview.video_port_base");
  ...
  if (cfg.optiview_video_port_base <= 0) {
    throw std::runtime_error("output.optiview.video_port_base must be > 0");
  }
  if (cfg.optiview_json_port_base <= 0) {
    throw std::runtime_error("output.optiview.json_port_base must be > 0");
  }
  if (cfg.save_every < 0) {
