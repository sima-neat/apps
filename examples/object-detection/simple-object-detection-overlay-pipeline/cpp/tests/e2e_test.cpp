...
int main(int argc, char** argv) {
  ...
  auto out_dir = create_temp_dir("simple-object-detection-overlay-pipeline_e2e_");
  if (out_dir.empty())
    return 1;

  auto config_dir = create_temp_dir("simple-object-detection-overlay-pipeline_config_");
  if (config_dir.empty()) {
    remove_dir(out_dir);
    return 1;
  }
  ...
  if (r.exit_code != 0) {
    ...
    std::cout << "[OK] object detection overlay produced " << count_files(out_dir)
              << " output files\n";
  }
