// Unit test for simple-object-detection-overlay-pipeline: validates CLI arg handling.
...
int main(int argc, char** argv) {
  ...
  // Test 2: only 2 args → exit 1
  {
    auto r = spawn_and_wait(binary, {"model.tar.gz", "labels.txt"}, 10000);
    if (r.exit_code != 1) {
  ...
  // Test 3: bad input dir → exit 2
  {
    auto r = spawn_and_wait(binary,
        {"model.tar.gz", "labels.txt", "/nonexistent_dir_abc", "/tmp/out"}, 10000);
    if (r.exit_code != 2) {
