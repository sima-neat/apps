...
# Ordered best-first. max_streams is the last count that ran clean; one more
# than that is where the run actually died, so these are hard limits.
#
#   2 x 2160p -> "Decoder plugin pool allocation failed"   (only 415 MP/s, so
#                this is decoder POOL MEMORY, not throughput)
#   6 x 1080p -> "Failed to allocate output buffer pool"
#   8 x  720p -> ran, but 19.8 fps against 25 fps sources
TIERS: list[Tier] = [
