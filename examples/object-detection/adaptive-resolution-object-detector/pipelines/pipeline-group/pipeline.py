...
# Per-pipeline files, so the deployments never clobber each other's state.
CONFIG = HERE / f"{PIPELINE}-run.yaml"     # unused in grouped mode; see config_path()
...
Path(LOG).parent.mkdir(parents=True, exist_ok=True)      # unused in grouped mode; see log_path()
