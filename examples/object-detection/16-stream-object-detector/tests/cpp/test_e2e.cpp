// E2E test for 16-stream-object-detector.
...
int main(int argc, char** argv) {
  ...
  const std::string model_path = configured_model_path("16-stream-object-detector", models_dir);
  if (model_path.empty() || !fs::exists(model_path)) {
    return skip_or_fail("configured detector model not found under SIMANEAT_APPS_TEST_MODELS_DIR");
  }

  const std::string output_dir = create_test_output_dir("16-stream-object-detector",
                                                        "test_16_stream_insight_and_save_pipeline");
  if (output_dir.empty()) {
    return 1;
  }
  ...
  const int total_saved_frames =
      e2e_int("16-stream-object-detector", "testing.e2e.output", "total_saved_frames");
  write_e2e_config("16-stream-object-detector", config_path,
  ...
  if (result.exit_code != 0) {
    ...
    if (files < total_saved_frames) {
      ...
      std::cout << "[OK] 16-stream object detector produced " << files << " sampled output files\n";
    }
  }
  if (rc == 0) {
