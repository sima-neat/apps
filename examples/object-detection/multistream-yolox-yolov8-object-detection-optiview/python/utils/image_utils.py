"""Image preprocessing and overlay helpers for the multistream detection example."""
...
from .sample_utils import class_label
...
def build_cpu_quanttess_preproc_state(runtime, contract: QuantTessCpuPreproc, src_width: int, src_height: int):
    np = runtime.np
...
def cpu_quanttess_input(runtime, frame_rgb, state: QuantTessCpuPreprocState):
    cv2 = runtime.cv2
    np = runtime.np
    ...
    resized = cv2.resize(frame_rgb, (state.scaled_w, state.scaled_h), interpolation=cv2.INTER_LINEAR)
...
def draw_detection_boxes(runtime, frame, detections: list[dict], class_labels: list[str]):
    cv2 = runtime.cv2
    for det in detections:
        class_id = int(det.get("class_id", -1))
        color = class_color(class_id)
        x1 = int(det.get("x1", 0.0))
        y1 = int(det.get("y1", 0.0))
        x2 = int(det.get("x2", 0.0))
        y2 = int(det.get("y2", 0.0))
        if x2 <= x1 or y2 <= y1:
            continue

        text = f"{class_label(class_labels, class_id)} {float(det.get('score', 0.0)):.2f}"
...
def save_debug_frame(runtime, output_dir: Path | None, stream_index: int, frame_index: int, frame, save_every: int) -> bool:
