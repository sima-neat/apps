// E2E test for single-image-object-detector.
...
int main(int argc, char** argv) {
  ...
  const std::string out_dir =
      create_test_output_dir("single-image-object-detector", "test_full_pipeline");
  if (out_dir.empty()) {
    return 1;
  }
  ...
  remove_dir(out_dir);
  std::cout << "[OK] single-image-object-detector pipeline completed successfully: " << out_image
            << "\n";
  return 0;
}
