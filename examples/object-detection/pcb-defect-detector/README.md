...
detector. The example reads every image in a folder, runs it through a compiled
BF16 model pack on the MLA, and writes one annotated image per input.
...
`model.path` in the example config already points at `models/yolo26n_plc_mpk.tar.gz`,
so no configuration change is needed after the download.
...
Edit `examples/object-detection/pcb-defect-detector/src/common/config.yaml`.

```yaml
model:
  path: models/yolo26n_plc_mpk.tar.gz
...
`runtime.profile: true` for per-image timing.
...
```bash
./examples/object-detection/pcb-defect-detector/src/cpp/pre-built/pcb-defect-detector \
  --config examples/object-detection/pcb-defect-detector/src/common/config.yaml
```
...
pip install -r examples/object-detection/pcb-defect-detector/src/python/requirements.txt
python3 examples/object-detection/pcb-defect-detector/src/python/main.py \
  --config examples/object-detection/pcb-defect-detector/src/common/config.yaml
```
