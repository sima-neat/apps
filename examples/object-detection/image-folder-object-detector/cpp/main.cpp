/**
 * @example image-folder-object-detector.cpp
 * Minimal YOLOv8n sync pipeline: infer detections for every image in a folder.
 *
 * Usage: image-folder-object-detector [--config <path>]
 */
#include "neat.h"
...
Config load_config(const fs::path& path) {
  ...
  cfg.labels_path =
      raw.string_or("model.labels",
                    "examples/object-detection/image-folder-object-detector/common/coco_label.txt");
  cfg.input_dir = raw.string_or("io.input_dir", "assets/test_images");
  cfg.output_dir = raw.string_or("io.output_dir", "sandbox/image-folder-object-detector");
