...
from .sample_utils import (
    Detection,
    build_insight_detection_payload,
    detections_from_detector_sample,
    insight_frame_id,
    insight_timestamp_ms,
...
def startup_trace_enabled_from_env() -> bool:
    raw = os.getenv("SIMA_INSIGHT_STARTUP_TRACE")
    if raw is None:
        return False
...
def format_video_build_error(stream_index: int, video_mode: VideoMode, detail: str) -> str:
    return (
        f"stream {stream_index} failed to build Insight "
...
def _initialize_stream_runtime(
    ...
    metadata_enabled = metadata_output_enabled(cfg)
...
def _process_frame(
    ...
    if stream.video_enabled:
        ...
        if stream.video is None:
            try:
                stream.video = build_insight_video_run(
                    stream.runtime,
                    cfg,
                    stream.probe,
                    stream.index,
    ...
    if stream.metadata_enabled and stream.metadata_sender is not None:
        ...
        metadata_t0 = _now_steady_s()
        data_json = json.dumps({"objects": payload.objects}, separators=(",", ":"))
        ok = stream.metadata_sender.send_metadata(
            "object-detection",
            data_json,
            insight_timestamp_ms(publish_wall_time_s, cfg.insight_metadata_offset_ms),
            insight_frame_id(det_sample, packet.frame_index),
        )
        if ok is False:
...
def _print_interval_profile(stream: StreamRuntime) -> None:
    ...
    print(
        ...
        f"metadata={stream.metrics.interval_metadata_s * 1000.0 / n:.6g}ms "
...
def _print_profile_summary(streams: list[StreamRuntime]) -> None:
    ...
    for stream in streams:
        ...
        print(
            ...
            f"metadata={stream.metrics.metadata_time_s * 1000.0 / n:.6g}ms "
...
def run_app(cfg: AppConfig, family: ModelFamily) -> int:
    ...
    for stream in streams:
        assert stream.probe is not None
        video = (
            str(insight_video_port_for_stream(cfg.insight_video_port_base, stream.index))
            if stream.video_enabled
            else "disabled"
        )
        metadata = (
            str(insight_metadata_port_for_stream(cfg.insight_metadata_port_base, stream.index))
        ...
        print(
            ...
            f"insight://{cfg.insight_host} video={video} metadata={metadata}",
            flush=True,
        )
