...
def insight_frame_id(sample: Any, fallback_frame_index: int) -> str:
...
def insight_timestamp_ms(publish_time_s: float, offset_ms: float) -> int:
    return int(round(publish_time_s * 1000.0 + offset_ms))


def build_insight_detection_payload(
    ...
    for det in detections:
        ...
        if x2 <= x1 or y2 <= y1:
            continue
        label = class_labels[det.class_id] if 0 <= det.class_id < len(class_labels) else "unknown"
        objects.append(
            {
    ...
    return InsightDetectionPayload(objects=objects)
