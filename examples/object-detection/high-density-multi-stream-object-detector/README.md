...
Detector liveness follows completed work rather than a fixed per-camera wall-clock rate. Startup requires two detections from every configured stream within `runtime.initial_detection_timeout_ms`; after that, a stream may miss up to `runtime.max_missed_detection_rounds * stream_count + inference.max_inflight_total` aggregate completions. Crossing that boundary is latched at the completion that exposes it, so a later result cannot hide an already-starved stream. This scales the fairness check with any supported stream shape while retaining enough headroom for work already in flight. `runtime.no_detection_timeout_ms` is a separate wall-clock guard that fires whenever the entire detector stops returning results, including during startup; an already-expired stall is latched before recovered progress, and a bounded frame target cannot bypass either failure.
