...
"""Create real moving, no-B-frame high-density test videos with unique channel markers."""
...
def ffmpeg_command(
    args: argparse.Namespace, channel: int, rgb: list[int], output: Path
) -> list[str]:
...
def main() -> int:
    ...
    outputs: dict[int, Path] = {
        channel: args.output_dir
        / f"channel-{channel:02d}-{args.width}x{args.height}p{args.fps}-no-b.mp4"
    ...
    manifest = {
        "version": 1,
        "description": (
            "high-density app real-moving fixtures with stable per-channel color and text markers"
        ),
        "width": args.width,
        "height": args.height,
        "fps": args.fps,
