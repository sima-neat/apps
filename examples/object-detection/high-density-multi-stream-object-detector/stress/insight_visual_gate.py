...
"""Validate moving high-density video and live boxes in a dedicated Insight tab.
...
to one viewer).  The target is closed before exit unless
``--keep-target-on-success`` is requested; no existing operator tab is
navigated or reused.
"""
...
(() => {
  ...
  const wrap = (name, field) => {
    ...
    if (!original || original.__neatHighDensityWrapped) return;
    function wrapped(...args) {
      const canvas = this && this.canvas;
      if (canvas) canvas[field] = (canvas[field] || 0) + 1;
      return original.apply(this, args);
    }
    wrapped.__neatHighDensityWrapped = true;
    proto[name] = wrapped;
  };
  wrap('clearRect', '__neatHighDensityClearOps');
  for (const name of ['stroke', 'strokeRect', 'fill', 'fillRect', 'fillText']) {
    wrap(name, '__neatHighDensityPaintOps');
  }
  ...
  window.__neatHighDensityVideoFrameMetadataFor = video => videoFrameMetadata.get(video) || null;

  window.__neatHighDensityMetadataByChannel = Object.create(null);
  const metadataIdentityStateByChannel = Object.create(null);
  window.__neatHighDensityPeers = [];
  ...
  const metadataIdentity = payload => {
    // PTS is the strongest frame identity in the App metadata contract. Fall
    // back to frame ID, then RTP timestamp for older/custom senders.
    if (Number.isFinite(payload.pts_ns)) return `pts:${payload.pts_ns}`;
    const frameId = identityValue(payload.frame_id);
    if (frameId !== null) return `frame:${frameId}`;
    if (Number.isFinite(payload.rtp_timestamp)) {
      return `rtp:${payload.rtp_timestamp}`;
    }
    return null;
  };
  const observeMetadataChannel = channel => {
    ...
    channel.addEventListener('message', event => {
      try {
        ...
        const previous = window.__neatHighDensityMetadataByChannel[stream] || {
          count: 0,
          uniqueCount: 0,
          duplicateCount: 0,
          unidentifiedCount: 0,
          uniquePtsCount: 0,
          uniqueFrameIdCount: 0,
          uniqueRtpTimestampCount: 0,
        };
        const state = metadataIdentityStateByChannel[stream] || {
          identities: new Set(),
          pts: new Set(),
          frameIds: new Set(),
          rtpTimestamps: new Set(),
        };
        metadataIdentityStateByChannel[stream] = state;
        const identity = metadataIdentity(payload);
        const pts = Number.isFinite(payload.pts_ns) ? payload.pts_ns : null;
        const frameId = identityValue(payload.frame_id);
        const rtpTimestamp = Number.isFinite(payload.rtp_timestamp)
          ? payload.rtp_timestamp : null;
        const duplicate = identity !== null && state.identities.has(identity);
        if (identity !== null) state.identities.add(identity);
        if (pts !== null) state.pts.add(pts);
        if (frameId !== null) state.frameIds.add(frameId);
        if (rtpTimestamp !== null) state.rtpTimestamps.add(rtpTimestamp);
        window.__neatHighDensityMetadataByChannel[stream] = {
          count: previous.count + 1,
          uniqueCount: state.identities.size,
          duplicateCount: previous.duplicateCount + (duplicate ? 1 : 0),
          unidentifiedCount: previous.unidentifiedCount + (identity === null ? 1 : 0),
          uniquePtsCount: state.pts.size,
          uniqueFrameIdCount: state.frameIds.size,
          uniqueRtpTimestampCount: state.rtpTimestamps.size,
  ...
  pcProto.setRemoteDescription = function(...args) {
    if (!remoteHookedPeers.has(this)) {
      ...
      window.__neatHighDensityPeers.push(this);
...
(() => {
  ...
  return Array.from(document.querySelectorAll('.video-tile')).map((tile, index) => {
    ...
    const presentedFrame = video && window.__neatHighDensityVideoFrameMetadataFor
      ? window.__neatHighDensityVideoFrameMetadataFor(video) : null;
    ...
    if (video && video.srcObject && video.srcObject.getVideoTracks) {
      ...
      if (track) {
        for (const pc of (window.__neatHighDensityPeers || [])) {
    ...
    return {
      ...
      metadata: channelId === null
        ? null
        : (window.__neatHighDensityMetadataByChannel[channelId] || null),
      overlayAlphaSamples,
      overlayPaintOps: canvas ? (canvas.__neatHighDensityPaintOps || 0) : 0,
      overlayClearOps: canvas ? (canvas.__neatHighDensityClearOps || 0) : 0,
    };
  });
})()
...
def metadata_cadence(
    ...
    transport_delta = counter_delta("count")
    unique_delta = counter_delta("uniqueCount")
    duplicate_delta = counter_delta("duplicateCount")
    unidentified_delta = counter_delta("unidentifiedCount")
    unique_pts_delta = counter_delta("uniquePtsCount")
    unique_frame_id_delta = counter_delta("uniqueFrameIdCount")
    unique_rtp_timestamp_delta = counter_delta("uniqueRtpTimestampCount")
    counters_consistent = (
        transport_delta is not None
        and unique_delta is not None
        and duplicate_delta is not None
        and unidentified_delta is not None
        and transport_delta == unique_delta + duplicate_delta + unidentified_delta
    )
    minimum_unique_messages = expected_fps * elapsed_s * minimum_fps_ratio

    def rate(delta: int | None) -> float | None:
        return delta / elapsed_s if delta is not None and elapsed_s > 0 else None

    return {
        # Explicit counters make it impossible to mistake repeated transport
        # messages for fresh box updates. Keep count_delta/fps as unique aliases
        # for existing report consumers.
        "count_delta": unique_delta,
        "fps": rate(unique_delta),
        "transport_count_delta": transport_delta,
        "transport_fps": rate(transport_delta),
        "unique_count_delta": unique_delta,
        "unique_fps": rate(unique_delta),
        "duplicate_count_delta": duplicate_delta,
        "duplicate_fps": rate(duplicate_delta),
        "unidentified_count_delta": unidentified_delta,
        "unidentified_fps": rate(unidentified_delta),
        "unique_pts_count_delta": unique_pts_delta,
        "unique_pts_fps": rate(unique_pts_delta),
        "unique_frame_id_count_delta": unique_frame_id_delta,
        "unique_frame_id_fps": rate(unique_frame_id_delta),
        "unique_rtp_timestamp_count_delta": unique_rtp_timestamp_delta,
        "unique_rtp_timestamp_fps": rate(unique_rtp_timestamp_delta),
        "counters_consistent": counters_consistent,
        "minimum_messages": minimum_unique_messages,
        "minimum_unique_messages": minimum_unique_messages,
        "passed": counters_consistent
        and unique_delta is not None
        and unique_delta >= minimum_unique_messages,
    }


def overlay_visibility(tile_samples: list[dict[str, Any]]) -> dict[str, Any]:
    visible_at_samples = [
        isinstance(sample.get("overlayAlphaSamples"), (int, float))
        and sample.get("overlayAlphaSamples", 0) > 0
        for sample in tile_samples
    ]
    visibility_count = sum(visible_at_samples)
    sample_count = len(visible_at_samples)
    return {
        "present": visibility_count > 0,
        "visible_at_samples": visible_at_samples,
        "visibility_count": visibility_count,
        "visibility_ratio": visibility_count / sample_count if sample_count else 0.0,
        "stable_at_samples": sample_count > 0 and visibility_count == sample_count,
    }
...
def analyze_temporal_samples(
    ...
    presented_metadata_rtp_offsets: list[int] = []
    ...
    all_metadata_valid = all(
        ...
        and isinstance(row.get("uniqueCount"), (int, float))
    ...
    if all_metadata_valid and all_video_rtp_valid:
        ...
        if all(
            isinstance(row.get("insightRtpTimestamp"), (int, float))
            for row in metadata
        ):
            for metadata_row, video_row in zip(metadata, video_rtp):
                presented_metadata_rtp_offsets.append(
                    signed_circular_delta(
                        int(video_row["rtpTimestamp"]) & 0xFFFFFFFF,
                        int(metadata_row["insightRtpTimestamp"]) & 0xFFFFFFFF,
                        1 << 32,
                    )
                )
    ...
    for index in range(1, len(tile_samples)):
        ...
        metadata_transport_count_delta = None
        metadata_unique_count_delta = None
        ...
        if all_metadata_valid:
            ...
            metadata_transport_count_delta = int(after_metadata["count"]) - int(
                before_metadata["count"]
            )
            metadata_unique_count_delta = int(after_metadata["uniqueCount"]) - int(
                before_metadata["uniqueCount"]
            )
        ...
        metadata_forward = (
            ...
            and metadata_unique_count_delta is not None
            and metadata_unique_count_delta > 0
            and rtp_matches_pts
        )
        segments.append(
            {
                ...
                "metadata_count_delta": metadata_unique_count_delta,
                "metadata_transport_count_delta": metadata_transport_count_delta,
                "metadata_unique_count_delta": metadata_unique_count_delta,
    ...
    presented_metadata_rtp_tolerance_ticks = round(
        sync_tolerance * 90_000 / fps
    )
    presented_metadata_rtp_alignment_stable = bool(
        presented_metadata_rtp_offsets
    ) and all(
        abs(offset) <= presented_metadata_rtp_tolerance_ticks
        for offset in presented_metadata_rtp_offsets
    )
    # Newer Insight versions stamp metadata with the outgoing video RTP
    # timestamp selected by the source-to-egress correlator. Prefer that exact
    # presented-clock comparison. The fixture-code/PTS comparison remains the
    # compatibility gate for older Insight versions that do not provide the
    # correlated timestamp.
    presented_alignment_source = (
        "insight-correlated-rtp"
        if presented_metadata_rtp_offsets
        else "fixture-code-vs-source-pts"
    )
    presented_alignment_stable = (
        presented_metadata_rtp_alignment_stable
        if presented_metadata_rtp_offsets
        else media_origin_offset_stable
    )
    ...
    return {
        ...
        "presented_metadata_rtp_offsets_ticks": presented_metadata_rtp_offsets,
        "presented_metadata_rtp_tolerance_ticks": (
            presented_metadata_rtp_tolerance_ticks
        ),
        "presented_metadata_rtp_alignment_stable": (
            presented_metadata_rtp_alignment_stable
        ),
        "presented_alignment_source": presented_alignment_source,
        "presented_alignment_stable": presented_alignment_stable,
        "segments": segments,
        ...
        and presented_alignment_stable,
    }
...
def main() -> int:
    ...
    try:
        ...
        for index, expected_id in enumerate(expected_ids):
            ...
            metadata_rate = metadata_cadence(
                tile_samples,
                elapsed,
                args.expected_fps,
                args.minimum_fps_ratio,
            )
            overlay_result = overlay_visibility(tile_samples)
            records.append(
                {
                    ...
                    "metadata_count_delta": metadata_rate["unique_count_delta"],
                    "metadata_fps": metadata_rate["unique_fps"],
                    "metadata_transport_count_delta": metadata_rate[
                        "transport_count_delta"
                    ],
                    "metadata_transport_fps": metadata_rate["transport_fps"],
                    "metadata_unique_count_delta": metadata_rate[
                        "unique_count_delta"
                    ],
                    "metadata_unique_fps": metadata_rate["unique_fps"],
                    "metadata_duplicate_count_delta": metadata_rate[
                        "duplicate_count_delta"
                    ],
                    "metadata_duplicate_fps": metadata_rate["duplicate_fps"],
                    "metadata_unidentified_count_delta": metadata_rate[
                        "unidentified_count_delta"
                    ],
                    "metadata_unique_pts_count_delta": metadata_rate[
                        "unique_pts_count_delta"
                    ],
                    "metadata_unique_frame_id_count_delta": metadata_rate[
                        "unique_frame_id_count_delta"
                    ],
                    "metadata_unique_rtp_timestamp_count_delta": metadata_rate[
                        "unique_rtp_timestamp_count_delta"
                    ],
                    "metadata_counters_consistent": metadata_rate[
                        "counters_consistent"
                    ],
                    "metadata_cadence_ok": metadata_rate["passed"],
                    "overlay_visible": overlay_result["stable_at_samples"],
                    "overlay_present": overlay_result["present"],
                    "overlay_visible_at_samples": overlay_result[
                        "visible_at_samples"
                    ],
                    "overlay_visibility_count": overlay_result[
                        "visibility_count"
                    ],
                    "overlay_visibility_ratio": overlay_result["visibility_ratio"],
                    "overlay_stable_at_samples": overlay_result[
                        "stable_at_samples"
                    ],
        ...
        checks = {
            ...
            "all_overlays_visible": all(
                record["overlay_stable_at_samples"] for record in records
            ),
            "all_overlays_redrawing": all(
                record["overlay_paint_delta"] > 0 for record in records
            ),
            "all_metadata_fps": all(record["metadata_cadence_ok"] for record in records),
        }
        if identity:
        ...
        if temporal_marker:
            ...
            checks["all_video_metadata_presented_alignment_stable"] = all(
                record["temporal"]
                and record["temporal"]["presented_alignment_stable"]
                for record in records
            )
        summary = {
            ...
            "minimum_metadata_messages": minimum_frames,
            "minimum_unique_metadata_messages": minimum_frames,
            "metadata_fps": rate_summary(
                [
                    float(record["metadata_fps"])
                    for record in records
                    if record["metadata_fps"] is not None
                ]
            ),
            "metadata_unique_fps": rate_summary(
                [
                    float(record["metadata_unique_fps"])
                    for record in records
                    if record["metadata_unique_fps"] is not None
                ]
            ),
            "metadata_transport_fps": rate_summary(
                [
                    float(record["metadata_transport_fps"])
                    for record in records
                    if record["metadata_transport_fps"] is not None
                ]
            ),
            "metadata_duplicate_fps": rate_summary(
                [
                    float(record["metadata_duplicate_fps"])
                    for record in records
                    if record["metadata_duplicate_fps"] is not None
                ]
            ),
            "metadata_rate_miss_channels": [
                record["expected_channel_id"]
                for record in records
                if not record["metadata_cadence_ok"]
            ],
