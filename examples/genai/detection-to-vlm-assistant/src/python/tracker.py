"""Compact two-stage, motion-aware tracker for browser person detections."""
...
def _normalized_center_distance(a: BBox, b: BBox) -> float:
    ...
    scale = max(
        1.0,
        0.5
        * (math.hypot(_width(a), _height(a)) + math.hypot(_width(b), _height(b))),
    )
...
class ObjectTracker:
    """Associate detections by score, IoU, and constant-velocity motion."""
